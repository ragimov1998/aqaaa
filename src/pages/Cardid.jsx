...
function Cardid() {
  ...
  return (
    <div className="cardid">
      ...
      {isdata.cardabout &&
        isdata.cardabout.map((item, index) => (
          <div key={index} className="text">
            <h1>{item.melumatlar}</h1>
            <h2>{item.basliq}</h2>
            {item.libasliq&&item.libasliq.map((i,x)=>(<li key={x} className="list">{i}</li>))}
            {item.cashback && <h2>{item.cashback}</h2>}
            {item.cashbackp && <p>{item.cashbackp}</p>}
            {item.cashli &&item.cashli.map((it,ind)=>(<li className="list" key={ind}>{it}</li>))}
            ...
            {item.faizli&&item.faizli.map((s,z)=>(<li className="list" key={z}>{s}</li>))}
            {item.unvan&&<h2>{item.unvan}</h2>}
            {item.market&&<h2>{item.market}</h2>}
            {item.marketler&&item.marketler.map((market,sayi)=>(<li key={sayi} className="list">{market}</li>))}
            {item.elektronik&&<h2>{item.elektronik}</h2>}  
            {item.magazalar&&item.magazalar.map((magaza,c)=>(<li className="list" key={c}>{magaza}</li>))} 
            ...
            {item.avtoli&&item.avtoli.map((ll,aa)=>(<li className="list" key={aa}>{ll}</li>))}
            {item.avtop&&<p>{item.avtop}</p>}
            {item.milhesab&&<h2>{item.milhesab}</h2>} 
            <div className="mil">
              <div className="left">
            {item.milli&&item.milli.map((millir,are)=>(<li className="lost list " key={are}>{millir}</li>))}</div>
              <div className="right">
              {item.millileft&&item.millileft.map((mi,ar)=>(<li className="lost" key={ar}>{mi}</li>))}
              </div>
